...
@Service("unlockCardProcessor")
public class UnlockCardProcessorImpl extends UnlockSdProcessorImpl {
	...
	protected void check(LocalTransaction localTransaction) {
		...
		if (cardSd == null || CardSecurityDomain.STATUS_LOCK != cardSd.getStatus().intValue()) {// 如果卡上安全域记录不存在或者不是“已个人化”状态，抛出异常
	...
	protected ApduCommand launchUnLock(LocalTransaction localTransaction) {
		...
		return apduEngine.buildSetStatusCmd(cms2acParam, (byte) 0x0F, false);
	}
