...
public class MobileWebServiceImpl implements MobileWebService {
		...
		try {
			if (req.getQueryCondition() != null && req.getQueryCondition().equals("commendApp")) {
				...
				if (isDownloaded.intValue() == 0
						|| (req.getQueryCondition() != null && req.getQueryCondition().equals("topDownload"))) {
			...
			case NotifyImsi: {
				...
				if (null == card || !card.getChallengeNo().equals(request.getChallengeNo())) {
					...
					if (null == customerCard) {// 如果绑定关系为空，说明终端未绑定，执行注册
						message = PlatformMessage.MOBILE_REGISTER;
					} else {// 如果绑定关心存在，首先判定是否在黑名单
						if (customerCard.isInBlackList()) {// 如果在黑名单
							message = PlatformMessage.MOBILE_IN_BLACK_LIST;
						} else {// 如果不再黑名单
							if (customerCard.getMobileNo().equals(card.getMobileNo())) {// 如果绑定手机号与上行短信手机号一致，执行修改IMSI操作
	...
	private void processTrans(LoginOrRegisterRequest request, ResExecAPDU resAPDU, ResLoginOrRegister res, String cardNo) {
		...
		appAids.add(operate);
		sessionId = transactionHelper.createSession(appAids, cardNo, request.getCommonType(),
				SpringSecurityUtils.getCurrentUserName());
		LocalTransaction localTransaction = localTransactionManager.getBySessionId(sessionId);
		localTransaction.setBeginTime(Calendar.getInstance());
		// 开始处理流程，结果保存在参数apdu中
