...
var LightFace = new Class({
	...
	draw : function() {
		...
		// draw rows and cells; use native JS to avoid IE7 and I6 offsetWidth
		// and offsetHeight issues
		...
		if (this.options.title) {
			var titleContent = '';
			if ($chk(this.options.titleImage)) {// 加个图片
				titleContent = '<img style="vertical-align:middle; margin:2px 5px 3px 0px; border:none;" src=' + this.options.titleImage
						+ ' />';
			}
			titleContent += this.options.title;
	...
	showButton : function(title) {
		if (this.buttons[title])
			this.buttons[title].removeClass('hiddenButton');
		return this.buttons[title];
	},
	hideButton : function(title) {
		if (this.buttons[title])
			this.buttons[title].addClass('hiddenButton');
		return this.buttons[title];
	},
	...
	// Repositions the box
	_position : function() {
		var windowSize = window.getSize(), scrollSize = window.getScroll(), boxSize = this.box.getSize();
		this.box.setStyles({
