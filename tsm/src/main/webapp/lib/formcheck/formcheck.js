...
var FormCheck = new Class({
	...
	options : {
		...
		alerts : {
			...
			en_cn : "Please enter english char or chinese char.",
		...
		regexp : {
			...
			phone : /^1[358]\d{9}$/, // alternate regex : /^[\d\s ().-]+$/,/^((\+\d{1,3}(-| )?\(?\d\)?(-| )?\d{1,5})|(\(?\d{2,6}\)?))(-| )?(\d{3,4})(-| )?(\d{4})(( x| ext)\d{1,5}){0,1}$/
			phone_inter : /^\+{0,1}[0-9 \(\)\.\-]+$/,
	...
	addError : function(obj) {
		...
		if(!obj.element && this.options.display.indicateErrors != 0) {
			if (this.options.display.errorsLocation == 1) {
				...
				obj.element = new Element('div', {'class' : this.options.tipsClass, 'styles' : options , 'id' : "error"+obj.id}).injectInside(document.body);
