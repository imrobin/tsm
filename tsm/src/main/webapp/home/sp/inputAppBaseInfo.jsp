...
<script src="${ctx}/lib/mootools/mootools-more.js"
	type="text/javascript"></script>
...
<script src="${ctx}/lib/lightface/LightFace.MessageBox.js"
	type="text/javascript"></script>
<script src="${ctx}/home/sp/js/inputAppBaseInfo.js"
	type="text/javascript"></script>
<script src="${ctx}/lib/ie6png.js" type="text/javascript"></script>
...
<script type="text/javascript">
								...
								'appBaseInfoForm',
								{
									...
									onValidateSuccess : function() {
										...
										new Request.JSON(
													...
													onSuccess : function(result) {
														if (result.success) {
															new LightFace.MessageBox()
																	.info("操作成功");
															self.location = ctx
																	+ "/home/sp/uploadCap.jsp?applicationVersionId="
																	+ result.message;
														} else {
															new LightFace.MessageBox()
												...
												}).post($('appBaseInfoForm')
												.toQueryString());
									}
								});

						$('appDesc').addEvent('keyup', function(event) {
							info.textCounter($('appDesc'));
						});

						$("fristType").addEvent("change", function(event) {
							event.stop();
							info.getSecondType();
						});
						...
						new SWFUpload(
								{
									...
									upload_success_handler : info.uploadPcIconCallback
											.bind(info),
						...
						new SWFUpload(
								{
									...
									upload_progress_handler : uploadProgress,
									upload_error_handler : uploadError,
									upload_success_handler : info.uploadMobileIconCallback
											.bind(info),
									upload_complete_handler : uploadComplete,
						...
						new SWFUpload(
								{
									...
									upload_progress_handler : uploadProgress,
									upload_error_handler : uploadError,
									upload_success_handler : info.uploadAppliationImageCallback
											.bind(info),
									upload_complete_handler : uploadComplete,
	...
	function checkAppAid(el) {
		return checkAid(el, ctx + "/html/application/?m=validateAid");
	}
</script>
</head>
		...
		<div id="main">
			<div class="div980 line_c">
				...
				<div class="usercont">
					...
					<div class="userinput">
						<div class="upapp_t">
							<a class="on">输入基本信息</a><a class="on3">上传加载文件</a> <a class="on3">上传应用扩展</a>
							<a class="on3">定义模块和实例</a> <a class="on3">配置顺序</a><a class="on3">上传客户端</a>
						</div>
						<div class="upapptext">
							<p class="left">第 1 步: 输入基本信息</p>
							<p class="right c_h">输入应用名称、描述等基本信息</p>
						</div>
						<form id="appBaseInfoForm"
							action="${ctx}/html/application/?m=createApplication">
							<table>
								<tbody>
									<tr>
									...
									<tr>
										...
										<td class="left inputs"><input id="appAid"
											class="inputtext validate['required','length[10,32]','%checkHex','%checkAppAid']"
											name="aid" type="text" maxlength="32" /></td>
									</tr>
									<tr>
										<td class="regtext"><span style="color: red">*</span>应用描述:</td>
										<td class="left inputs"><textarea id="appDesc"
												class="inputtext validate['required','length[0,256]']"
												name="description"></textarea>
										</td>
									</tr>
									<tr>
										<td class="regtext"><span style="color: red">*</span>应用版本:</td>
										<td class="left inputs"><input
											class="inputtext validate['required','%checkVersionNo']"
											name="versionNo" type="text" value="1.0.0" maxlength="8"
											style="width: 100px; vertical-align: middle;" /><span
											class="explain">格式为x.x.x，x代表0-99的数字</span></td>
									</tr>
									<tr>
									...
									<tr>
										...
										<td class="left inputs"><select name="personalType">
												...
												<option selected="selected" value=1>指令透传</option>
									...
									<tr>
										...
										<td class="left inputs"><select id="sdModel"
											name="sdModel">
												<option value=1 selected="selected">主安全域</option>
									...
									<tr>
										...
										<td class="left inputs"><input
											...
											name="businessPlatformUrl" type="text" maxlength="255" />
										</td>
									</tr>
									<tr>
										<td class="regtext"><span style="color: red">*</span>业务平台服务名:</td>
										<td class="left inputs"><input
											class="inputtext validate['required']" name="serviceName"
									...
									<tr>
										...
										<td class="left inputs"><select name="deleteRule">
												...
												<option value=1 selected="selected">全部删除</option>
									...
									<tr id="pcIconUpload">
										<td class="regtext"><span style="color: red">*</span>PC版图标:</td>
										<td class="left inputs"><input name="pcIconFileName"
											type="text" class="inputtext validate['required']"
											readonly="readonly" /><span class="left explain">支持格式.jpg
												.png，建议尺寸128*128</span></td>
										<td><div
									...
									<tr id='pcIconDisplay' style="display: none;">
										<td class="regtext"></td>
										<td class="left inputs"><img id="pcIconImg"
											...
											name="pcIconTempFileAbsPath" type="hidden" /></td>
									</tr>
									<tr id="mobileIconUpload">
										<td class="regtext"><span style="color: red">*</span>手机版图标:</td>
										<td class="left inputs"><input name="mobileIconFileName"
											type="text" class="inputtext validate['required']"
											readonly="readonly" /><span class="left explain">支持格式.jpg
												.png，尺寸必须是50*50</span></td>
										<td>
											<div
												style="display: inline; border: solid 1px #7FAAFF; background-color: #C5D9FF; padding: 2px;">
											...
											<div id="mobileIconProgressContainer"
												style="height: 75px; display: none;"></div></td>
									</tr>
									<tr id="mobileIconDisplay" style="display: none">
										<td class="regtext"></td>
									...
									<tr id="applicationImgUpload">
										<td class="regtext"><span style="color: red">*</span>应用截图:</td>
										<td class="left inputs"><input
											name="applicationImgFileName" type="text"
											class="inputtext validate['required']" readonly="readonly" /><span
											class="left explain">支持格式.jpg .png，建议尺寸190*280（可以上传多张）</span>
										</td>
										<td>
											<div
												style="display: inline; border: solid 1px #7FAAFF; background-color: #C5D9FF; padding: 2px;">
								...
								<tfoot>
									<tr class="s">
										<td colspan="3" style="text-align: center;"><a
											class="subbutt validate['submit']" style="float: right;"><span>提交</span>
										</a>
										</td>
									</tr>
								</tfoot>
