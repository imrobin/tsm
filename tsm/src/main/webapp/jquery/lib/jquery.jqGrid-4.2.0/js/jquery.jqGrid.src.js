...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		sortData = function (index, idxcol,reload,sor){
			...
			//不以index，根据sortIndex或者name
			ts.p.sortname = ts.p.colModel[idxcol].sortIndex || ts.p.colModel[idxcol].name || index;
			so = ts.p.sortorder;
			//组合排序
			ts.p.sortname = ts.p.sortname + "_" + so;
			...
			if(edtrul.custom === true) {
				if( !(rqfield === false && this.isEmpty(val)) ) {
					if($.isFunction(edtrul.custom_func)) {
						var ret = edtrul.custom_func.call(g,val,nm,g.p.colModel[valref]);
						if($.isArray(ret)) {
							return ret;
						} else {
